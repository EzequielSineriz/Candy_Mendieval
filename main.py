...
fuente2 = pygame.font.SysFont("Candycrush/MedievalSharp.ttf", 30)
ingreso = ""
...
posicion_imagen =[0,0]
#Cargamos la imagen para mostrar el score
pantalla_score = pygame.image.load("Candycrush/imagenes/fondo_score.jpg")
pantalla_score = pygame.transform.scale(pantalla_score,(800,600))
posicion_imagen =[0,0]
#Carganis la imagen del rectangulo
rectangulo_score = pygame.image.load("Candycrush/imagenes/rectangulo_score.png")
rectangulo_score = pygame.transform.scale(rectangulo_score,(400,500))
posicion_rectangulo_score =[175,25]
# cargo la imagen del trofeo
trofeo = pygame.image.load("Candycrush/imagenes/trofeo_score.png")
trofeo = pygame.transform.scale(trofeo,(75,75))
posicion_trofeo =[330,30]
...
#Cargo sonido del score
sonido_score = pygame.mixer.Sound("Candycrush/sonidos/66.mp3")
sonido_score.set_volume(volumen)
...
string_ganador = ""
texto_win = fuente.render(string_ganador, True, constantes.RED1) #transforma el texto en una imagen
string_perdedor = ""
texto_fall = fuente.render(string_perdedor,True, constantes.RED1)

jugador1 = []
flag_fondo_score = False
resultado = bool
flag_perdedor = False
while run == True:
...
################################# PARTE GRILLA CON COLICION DE PIEZAS #########################################################
            conteo = bool #Variable necesaria para el tiempo
 
        if event.type == pygame.USEREVENT and en_menu == False:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if en_menu == False:  # Solo detectamos clics en el tablero si estamos fuera del menu
        ...
        if event.type == pygame.KEYDOWN and en_menu == False : #Aca tenemos que aramar otro evento para que reaccione a 
            if event.key == pygame.K_BACKSPACE:
                ingreso = ingreso[0:-1]
            else:
                ingreso += event.unicode
        
        if event.type == pygame.MOUSEBUTTONDOWN and en_menu == False:
            rect_boton_siguiente = pygame.Rect(posicion_siguiente_cuadro[0],posicion_siguiente_cuadro[1], 100, 25)
            if rect_boton_siguiente.collidepoint(event.pos) :
                print(f"el nombre guardado del ganador es: {ingreso}")
                puntos = 100
                jugador1 = [ingreso, puntos]
                texto_win = generar_csv("Candycrush\JugadoresCandyPygame.csv", jugador1)
                flag_fondo_score = True
                flag_resultado = True
            if rect_boton_siguiente.collidepoint(event.pos) :
                print(f"El nombre del perdedor es {ingreso}")
                puntos = 0
                jugador1 = [ingreso, puntos]
                texto_fall = generar_csv("Candycrush\JugadoresCandyPygame.csv", jugador1)
                flag_fondo_score = True
 ...
 ############################################ 1- Parte para verificar estados de pantalla y blintear Pantalla inicial ####################################
    ...
    ############################################## 2- Juego en grilla y ganadores o perdedores blinteo #####################################
    else:
 ...
 ######################################### 3- Mostrar el Score y el csv #############################################################

    if flag_fondo_score == True:
        sonido_score.play()
        ventana.blit(pantalla_score,posicion_imagen)
        ventana.blit(rectangulo_score,posicion_rectangulo_score)
        ventana.blit(trofeo,posicion_trofeo)
        if flag_resultado == True :
            texto_renderizado = fuente2.render(texto_win, True, (255, 255, 255))  # Blanco
            ventana.blit(texto_renderizado, (215, 140))
        if flag_resultado == False :
            texto_renderizado = fuente2.render(texto_fall, True, (255, 255, 255))  # Blanco
            ventana.blit(texto_renderizado, (215, 140))
